...
namespace HideoutTV
{
    ...
    new ConfigDescription(
        ...
        {
            if (tvPlayer == null || tvPlayer.length <= 0.0)
            {
                timestampText = "";
                return;
            }

            int current = (int)tvPlayer.time;
            int total = (int)tvPlayer.length;
            ...
            timestampText =
                currentMinutes +
                ":" +
                currentSeconds.ToString("00") +
                " / " +
                totalMinutes +
                ":" +
                totalSeconds.ToString("00");
        }
